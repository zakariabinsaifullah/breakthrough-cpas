...
if ( ! function_exists( 'btcpa_get_shortcodes' ) ) :
	...
	function btcpa_get_shortcodes() {
		return array(
			array(
				'title'       => __( 'Posts Grid', 'breakthrough-cpas' ),
				'tag'         => 'btcpa_posts_grid',
				'description' => __( 'Renders a filterable, AJAX-paginated post grid with category filter tabs.', 'breakthrough-cpas' ),
				'example'     => '[btcpa_posts_grid per_page="9" post_type="post" categories="4,9"]',
				'attrs'       => array(
					array( 'name' => 'per_page',  'default' => '6',     'desc' => __( 'Number of posts to show per page.', 'breakthrough-cpas' ) ),
					array( 'name' => 'post_type', 'default' => 'post',  'desc' => __( 'WordPress post type slug.', 'breakthrough-cpas' ) ),
					array( 'name' => 'categories','default' => '(all)', 'desc' => __( 'Comma-separated category IDs. Leave empty to include all.', 'breakthrough-cpas' ) ),
					array( 'name' => 'id',        'default' => '(none)','desc' => __( 'Grid ID for remote tab connection via <code>[btcpa_posts_tabs]</code>.', 'breakthrough-cpas' ) ),
				),
			),
			array(
				'title'       => __( 'Posts Tabs (Remote)', 'breakthrough-cpas' ),
				'tag'         => 'btcpa_posts_tabs',
				'description' => __( 'Outputs standalone filter tabs that control a <code>[btcpa_posts_grid id="…"]</code> placed anywhere else on the same page.', 'breakthrough-cpas' ),
				'example'     => '[btcpa_posts_tabs for="blog" categories="4,9" post_type="post"]',
				'attrs'       => array(
					array( 'name' => 'for',       'default' => '(required)', 'desc' => __( 'Must match the <code>id</code> attribute of the target grid.', 'breakthrough-cpas' ) ),
					array( 'name' => 'categories','default' => '(all)',      'desc' => __( 'Comma-separated category IDs shown as filter tabs.', 'breakthrough-cpas' ) ),
					array( 'name' => 'post_type', 'default' => 'post',       'desc' => __( 'Post type slug — must match the target grid.', 'breakthrough-cpas' ) ),
				),
			),
		);
	}
...
add_action( 'admin_enqueue_scripts', 'btcpa_shortcodes_admin_assets' );

if ( ! function_exists( 'btcpa_shortcodes_admin_assets' ) ) :
	function btcpa_shortcodes_admin_assets( $hook ) {
		if ( 'appearance_page_btcpa-shortcodes' !== $hook ) {
			return;
		}

		wp_enqueue_script(
			'btcpa-shortcodes-copy',
		...
		<div class="wrap psr-wrap">
			...
			<div class="psr-header">
				...
				<div class="psr-header__text">
					...
					<p><?php esc_html_e( 'All shortcodes available in this theme. Click Copy to grab the code.', 'breakthrough-cpas' ); ?></p>
				</div>
			</div>
			...
			<div class="psr-grid">
				...
				<div class="psr-card">
					...
					<div class="psr-card__body">
						<p class="psr-example-label"><?php esc_html_e( 'Example', 'breakthrough-cpas' ); ?></p>
						...
						<p class="psr-attrs-label"><?php esc_html_e( 'Attributes', 'breakthrough-cpas' ); ?></p>
						<div class="psr-attrs">
							<div class="psr-attr-head">
								<span><?php esc_html_e( 'Attribute', 'breakthrough-cpas' ); ?></span>
								<span><?php esc_html_e( 'Default', 'breakthrough-cpas' ); ?></span>
								<span><?php esc_html_e( 'Description', 'breakthrough-cpas' ); ?></span>
							</div>
			...
			<div class="psr-footer">
				<strong><?php esc_html_e( 'Tip:', 'breakthrough-cpas' ); ?></strong>
				<?php esc_html_e( 'Shortcodes can be placed in any post, page, or widget that supports shortcodes. In the block editor, use the Shortcode block.', 'breakthrough-cpas' ); ?>
			</div>
