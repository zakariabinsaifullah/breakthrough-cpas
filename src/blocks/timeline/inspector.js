...
const Inspector = props => {
    ...
    const inheritedHelp = gaps =>
        'Desktop' !== resMode && undefined === gaps?.[resMode]
            ? __('Inherited from the larger screen size. Change it to set a gap just for this device.', 'breakthrough-cpas')
            : undefined;

    return (
        <InspectorControls>
            <PanelBody title={__('Timeline Settings', 'breakthrough-cpas')} initialOpen={true}>
                <NativeResponsiveControl label={__('Gap (between items)', 'breakthrough-cpas')} props={props}>
                    <RangeControl
                ...
                <NativeResponsiveControl label={__('Gap (icon → content)', 'breakthrough-cpas')} props={props}>
                    <RangeControl
